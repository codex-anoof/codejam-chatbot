...
class ReferenceAgent:
    ...
    def extract_sections(self, docs):
        sections = []
        for doc in docs:
            for line in doc.page_content.split("\n"):
                if any(keyword in line for keyword in ["Chapter", "CHAPTER", "Section", "SECTION", "Activity", "Activity", "Coal Industry"]):
                    clean_line = line.strip()
                    if clean_line and clean_line not in sections:
                        sections.append(clean_line)
        return sections[:2] if sections else ["N/A"]
...
class Orchestrator:
    ...
    def run(self):
        ...
        for q in tqdm(self.queries):
            ...
            context_trimmed = context[:400] + "..." if len(context) > 400 else context
            answer = self.ans_agent.answer(q["question"], retriever.retriever)
            pages = self.ref_agent.extract_pages(docs)
            sections = self.ref_agent.extract_sections(docs)
        ...
        pd.DataFrame(results).to_csv("lordofcode.json.csv", index=False)
        print("✅ lordofcode.json.csv generated!")
